...
public interface PlanMapper {
    ...
    List<Plan> getAllPlans(@Param("id") String id);
    @Select("SELECT * FROM plans WHERE usr_id = #{id} AND done = false ORDER BY date;")
    List<Plan> getPlans(@Param("id") String id);
    @Select("SELECT * FROM plans WHERE usr_id = #{id} AND done = true ORDER BY date;")
    List<Plan> getHisPlans(@Param("id") String id);
    @Insert("INSERT INTO plans (plan_id , usr_id, content, link, date) VALUES (#{plan_id},#{id}, #{content}, #{link}, #{date}) ON DUPLICATE KEY UPDATE content = #{content} AND link = #{link} AND date = #{date};")
    ...
    @Insert("UPDATE plans SET done = true WHERE plan_id = #{plan_id};")
    void addHisPlan( @Param("plan_id")String plan_id);
}
