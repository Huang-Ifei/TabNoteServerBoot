...
public interface HomeworkSubmissionMapper {
    ...
    @Select("select hs.*, u.name as student_name from homework_submission hs inner join student s on hs.student_id = s.student_id left join user u on s.usr_id = u.id where hs.homework_id = #{0} order by hs.submit_time desc")
